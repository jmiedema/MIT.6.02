...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if virus_odds <= (self.maxBirthProb * (1 - popDensity)):
            ...
            # create new dictionary for new instance
            new_dict = {}

            # iterate through drugs
            for drug in activeDrugs:

                if isResistantTo(drug) == True:

                    # create odds of inheritence
                    inherit_odds = random.random()

                    if rep_odds <= (1-self.mutProb):
                        new_dict[drug] = True
                    else: 
                        new_dict[drug] = False

                else:
                    # create odds for having new resistence
                    new_res_odds = random.random()

                    if new_res_odds <= self.mutProb:
                        new_dict[drug] = True
                    else:
                        new_dict[drug] = False

            new_instance = ResistantVirus(self.maxBirthProb, self.clearProb, new_dict, self.mutProb)
            return new_instance
        
        else:
            print "not reproduce 2"
            return False
...
virus_list = [ResistantVirus(0.1, 0.05, {"guttagonol" : False}, 0.05) for x in range(0,100)]
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    return len(new_patient.viruses)

simulationWithDrug(100, 1000, 0.1, 0.05, {"guttagonol" : False}, 0.05, 1)
